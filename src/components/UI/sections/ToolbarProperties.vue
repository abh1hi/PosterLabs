...
import { 
  AlignLeft, ChevronDown, ChevronRight, Type, Search, Globe, MonitorDown, 
  Maximize2, ImageIcon, Crop, Wand2, Shapes, Palette, Smartphone, Copy 
...
const { elements, selectedId, updateElement, updateStyle } = useElements()
...
const applyTuiFilter = (filterType: string) => {
    ...
    let options: any = {}
    switch (filterType) {
        case 'Blur': options = { blur: 0.1 }; break
        case 'Pixelate': options = { blocksize: 10 }; break
    }
    tuiInstance.value.applyFilter(filterName, options).catch((e: any) => {
...
const radiusValues = computed(() => {
    ...
    if (typeof val === 'string') {
        // Parse "10px 20px 30px 40px" or "10px"
...
const updateRadius = (corner: 'all' | 'tl' | 'tr' | 'br' | 'bl', value: number) => {
    ...
    if (corner === 'all') {
        // Simple numeric update for uniform
         handleInput(selectedId.value, 'borderRadius', value, true)
    } else {
        // Construct string for individual
        const current = radiusValues.value
        const newVals = { ...current, [corner]: value }
        // Ensure we switch to string mode if we were simpler
        if (isUniformRadius.value) isUniformRadius.value = false
        
        // standard order: top-left, top-right, bottom-right, bottom-left
        const str = `${newVals.tl}px ${newVals.tr}px ${newVals.br}px ${newVals.bl}px`
        handleInput(selectedId.value, 'borderRadius', str, true)
    }
}
</script>
        ...
        <div v-if="selectedElement" class="flex flex-col">
                ...
                <div v-if="openSections.typography" class="space-y-4 px-4 pb-4">
                    ...
                    <div class="space-y-2"> 
                        <div class="flex items-center gap-2">
                            ...
                            >
                                <MonitorDown :size="20" /> <!-- Changed icon to avoid reusing Link/Monitor conflict? Original was Link, but MonitorDown was imported. Using MonitorDown for system fonts button below. -->
                            </button>
                        </div>
            ...
            <div v-if="selectedElement.type === 'image'" class="section-container">
                ...
                <div v-if="openSections.image" class="space-y-4 px-4 pb-4">
                     <!-- Image Mode: Fit vs Fill -->
                     <div class="flex flex-col gap-2 p-2 bg-surface-high rounded-xl border border-outline/10">
                        <div class="flex items-center justify-between">
                            <div class="flex flex-col">
                                <span class="label-medium">Fill Container</span>
                                <span class="text-[10px] text-on-surface-variant">Crop to fit shape</span>
                            </div>
                             <md-switch 
                                :selected="selectedElement!.style.objectFit === 'cover'" 
                                @change="(e: any) => handleInput(selectedId!, 'objectFit', e.target.selected ? 'cover' : 'contain', true)"
                            ></md-switch>
                        </div>
                    ...
                    <div class="space-y-2">
                         <h4 class="label-small uppercase text-on-surface-variant font-bold">Source URL</h4>
                         <div class="flex items-center gap-2 bg-surface-high rounded-xl p-2 border border-outline/10 focus-within:border-primary transition-colors">
                            <input 
                                type="text" 
                                :value="selectedElement!.src" 
                                @change="(e: any) => handleInput(selectedId!, 'src', e.target.value)"
                                class="flex-1 text-xs font-mono bg-transparent border-none outline-none text-on-surface placeholder-on-surface-variant/50"
                                placeholder="https://..."
                            />
                            <button @click="copyToClipboard(selectedElement!.src!)" class="p-1.5 hover:bg-surface-variant rounded-lg text-on-surface-variant transition-colors" title="Copy URL">
                                <Copy :size="14" />
                            </button>
                         </div>
                    </div>
                    ...
                    <div v-if="isCropping" class="flex flex-col gap-3">
                         <div class="h-64 bg-black/50 rounded-lg overflow-hidden relative">
                            <Cropper v-if="selectedElement!.src" ref="cropperRef" class="cropper" :src="selectedElement!.src" :stencil-props="{ aspectRatio: 0 }" />
                         </div>
                         <div class="grid grid-cols-2 gap-2">
            ...
            <div v-if="selectedElement.type !== 'text'" class="section-container">
                ...
                <div v-if="openSections.appearance" class="space-y-4 px-4 pb-4">
                    ...
                    <md-outlined-select label="Blend" class="w-full" :value="selectedElement!.style.mixBlendMode || 'normal'" @change="(e: any) => handleInput(selectedId!, 'mixBlendMode', e.target.value, true)">
                     ...
                     <div class="space-y-2">
                        <div class="flex justify-between items-center">
                            <span class="label-small">Corner Radius</span>
                            <div class="flex bg-surface-high rounded-lg p-0.5 border border-outline/10">
                                 <button 
                                    @click="isUniformRadius = true" 
                                    class="p-1 rounded-md transition-colors" 
                                    :class="isUniformRadius ? 'bg-primary text-on-primary shadow-sm' : 'text-on-surface-variant hover:bg-surface-variant'" 
                                    title="Uniform"
                                 >
                                    <div class="w-3 h-3 border-2 border-current rounded-sm"></div>
                                 </button>
                                 <button 
                                    @click="isUniformRadius = false" 
                                    class="p-1 rounded-md transition-colors" 
                                    :class="!isUniformRadius ? 'bg-primary text-on-primary shadow-sm' : 'text-on-surface-variant hover:bg-surface-variant'" 
                                    title="Individual"
                                 >
                                    <div class="w-3 h-3 border-l-2 border-t-2 border-current rounded-tl-sm"></div>
                                 </button>
                            </div>
                        </div>
                        ...
                        <div v-else class="grid grid-cols-2 gap-2">
                             <div class="space-y-1">
                                <span class="label-small text-[10px] uppercase opacity-70">Top Left</span>
                                <md-filled-text-field type="number" :value="String(radiusValues.tl)" @input="(e: any) => updateRadius('tl', parseFloat(e.target.value))"></md-filled-text-field>
                             </div>
                             <div class="space-y-1">
                                <span class="label-small text-[10px] uppercase opacity-70">Top Right</span>
                                <md-filled-text-field type="number" :value="String(radiusValues.tr)" @input="(e: any) => updateRadius('tr', parseFloat(e.target.value))"></md-filled-text-field>
                             </div>
                             <div class="space-y-1">
                                <span class="label-small text-[10px] uppercase opacity-70">Bottom Left</span>
                                <md-filled-text-field type="number" :value="String(radiusValues.bl)" @input="(e: any) => updateRadius('bl', parseFloat(e.target.value))"></md-filled-text-field>
                             </div>
                             <div class="space-y-1">
                                <span class="label-small text-[10px] uppercase opacity-70">Bottom Right</span>
                                <md-filled-text-field type="number" :value="String(radiusValues.br)" @input="(e: any) => updateRadius('br', parseFloat(e.target.value))"></md-filled-text-field>
                             </div>
                        </div>
                     </div>
